...
parser.add_argument('--output', default='CUB-PS-P', type=str, metavar='DIRECTORY',
                    help='name of output')
parser.add_argument('--train_test', default=0, choices=[0, 1], type=int, metavar='FLAG',
                    help="run the train_test function")
...
def get_loss(ls_lmbd2, ls_lmbd3, criterion, output, target):
    return (ls_lmbd2 * criterion(output[0], target) + ls_lmbd3 * criterion(output[1], target)) / (
            ls_lmbd2 + ls_lmbd3)


def get_avg_output(lmbd2, lmbd3, output):
    return (lmbd2 * output[0] + lmbd3 * output[1]) / (lmbd2 + lmbd3)
...
def train(config, model, optimizer, criterion, train_loader, epoch, lr_period, start_batch_idx):
    ...
    for i, (inputs, target) in enumerate(train_loader):
        ...
        loss = get_loss(config['ls_coef_part'], config['ls_coef_bi'], criterion, output, target)
        avg_output = get_avg_output(config['coef_part'], config['coef_bi'], output)
...
def test(config, model, criterion, val_loader):
    ...
    with torch.no_grad():
        ...
        for i, (loader_input, target) in enumerate(val_loader):

            loader_input = loader_input.cuda(non_blocking=True)
            target = target.cuda(non_blocking=True)

            output = model(loader_input)
            ...
            if i % config['print_freq'] == 0:
                print('Test: [{0}/{1}] '
                      ...
                      'Loss {loss.val:.4f} (avg: {loss.avg:.4f}) '
                      'Prec@1 {top1.val:.3f} (avg: {top1.avg:.3f}) '
                      'Class avg {class_avg.avg:.3f} '.
                      format(i, len(val_loader), batch_time=batch_time, loss=losses, class_avg=class_avg, top1=top1))
...
def train_test(config, model, optimizer, criterion, train_loader, valid_loader):
    ...
    for epoch in range(config['start_epoch'], config['epochs']):
        ...
        train_acc, train_top1, train_loss = train(config, model, optimizer, criterion, train_loader, epoch, lr_period,
                                                  start_batch_idx)
...
def main():
    ...
    datasets = utils.split_by_class(config, examples, labels, np.loadtxt(config['attributes_file']), class_map)
